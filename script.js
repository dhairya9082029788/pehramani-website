...
async function fetchProducts() {
    try {
        ...
        console.log(
            `SUCCESS: Loaded ${products.length} products into memory.`
        );
        ...
        // CLEAN URL ROUTING
        // Opens the correct page if somebody directly enters
        // pehramani.com/earrings
        // pehramani.com/shop
        // pehramani.com/product/PRODUCT-ID
...
function renderFooterDynamicShopList(list) {
    ...
    const footerShopList =
        document.getElementById(
            'footer-dynamic-shop-list'
        );
        ...
        <li>
            <a
                ...
                onclick="
                    ...
                    triggerFooterCategoryFilter('all', event);
                "
            >
                All Collections
            </a>
...
// Dedicated trigger that forces:
// routing
// filtering
// button states
window.triggerFooterCategoryFilter =
(categoryName) => {

    const targetCatLower =
        categoryName
            .toLowerCase()
            .trim();
...
function generateProductCardHTML(p) {
    ...
    const multiBadgeHTML =
        hasMultiple

            ? `
                <div
                    class="genz-multi-badge"
                    id="badge-${p.id}"
                >
                    1/${imgList.length} ✨
                </div>
...
function renderProductDetail(product) {
    ...
    if (
        ...
        productImages =
            [
                'https://via.placeholder.com/800x800?text=No+Image+Found'
            ];
            ...
            >
                ...
                >
                    ...
                    ${escapeHTML(
                        ...
                        'A curated Pehramani statement piece designed to elevate your everyday and occasion looks.'
...
async function(e) {
    ...
    try {
        ...
        alert(
            'Thank you for subscribing to Pehramani alerts! ✨'
        );
...
(id) => {
    ...
    if (!product) {
        ...
        return alert(
            'Product details are loading. Please try again.'
        );
...
() => {
    ...
    if (!locationInput) {
        ...
        alert(
            'Delivery Address is strictly required to verify routing coordinates!'
        );
...
function handleCardSwipeGesture(
    productId
) {
...
(
    ...
    const frame =
        document.querySelector(
            `[data-images][ontouchstart*="${productId}"]`
        );
...
(direction) => {
    ...
    const thumbs =
        layout.querySelectorAll(
            '.pdp-gallery-wrapper div[onclick]'
        );
